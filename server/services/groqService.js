...
async function streamChatCompletion(systemPrompt, userPrompt, onChunk, apiKey = '', model = DEFAULT_MODEL, responseFormat) {
  const params = {
    model: MODELS[model] ? model : DEFAULT_MODEL,
    messages: [
      { role: 'system', content: systemPrompt },
  ...
  const stream = await getGroqClient(apiKey).chat.completions.create(params);
...
// 1. Error Explanation
async function explainError(code, error, language, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are a coding mentor. Analyze errors and explain them simply. Always respond in valid JSON.`;
  const userPrompt = `The user wrote code in <language>${language}</language> and got this error:
...
Respond in this EXACT JSON format:
{
  ...
  if (onChunk) {
    return streamChatCompletion(systemPrompt, userPrompt, onChunk, apiKey, model, { type: 'json_object' });
  }
  return chatCompletion(systemPrompt, userPrompt, apiKey, model);
}

// 2. Code Fix
async function fixCodeAI(code, error, language, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are a code repair expert. Fix this code while keeping the user's logic intact. Return ONLY the corrected code. Do NOT wrap it in markdown. Do not say "Here is the code". CRITICAL: Do NOT output any <think> tags, do NOT explain your reasoning. Just output the raw code.`;
  const userPrompt = `Fix this <language>${language}</language> code:
...
${error || 'No specific error, but optimize and fix any issues.'}
  ...
  if (onChunk) {
    return streamChatCompletion(systemPrompt, userPrompt, onChunk, apiKey, model);
  }

  const response = await chatCompletionText(systemPrompt, userPrompt, apiKey, model);
...
// 3. Logic Explanation
async function explainLogicAI(code, language, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are a CS tutor. Explain code step-by-step. Always respond in valid JSON.`;
  const userPrompt = `Explain this <language>${language}</language> code step-by-step:
...
async function generateTestsAI(code, language, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are a QA engineer. Generate test cases. Always respond in valid JSON.`;
  const userPrompt = `Generate test cases for this <language>${language}</language> function:
...
async function auditCodeAI(code, language, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are a senior application security reviewer and refactoring coach. Audit code for exploitable security risks, reliability hazards, memory/resource leaks, and unsafe architecture. Always respond in valid JSON.`;
  const userPrompt = `Audit this <language>${language}</language> code:
...
async function visualizeAI(code, language, input = '', apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are a code tracer. Trace through code step by step showing variable states. Always respond in valid JSON.`;
  const userPrompt = `Trace through this <language>${language}</language> code step by step. Show variable states after each line.
...
async function explainCodeSnippetAI(code, language, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are an expert programming tutor. When a user highlights a snippet of code, explain what it does in simple, beginner-friendly language. Always respond in valid JSON.`;
  const userPrompt = `Explain this <language>${language}</language> code snippet in simple terms:
...
async function askFollowUpAI(code, language, question, previousExplanation, apiKey = '', model = DEFAULT_MODEL, onChunk = null) {
  const systemPrompt = `You are an expert programming tutor engaged in an interactive Q&A session. The user previously highlighted code and received an explanation. Now they have a follow-up question. Answer clearly and concisely. Always respond in valid JSON.`;
  const userPrompt = `The user is asking about this <language>${language}</language> code:
...
async function analyzeComplexityAI(code, language, apiKey = '', onChunk = null) {
  const systemPrompt = `You are an expert algorithms professor and competitive programmer. Analyze code for time and space complexity using Big-O notation. Be precise and accurate. Always respond in valid JSON.`;
  const userPrompt = `Analyze the time and space complexity of this ${language} code:
...
- If the code has multiple functions, analyze the overall combined complexity.`;
  ...
  if (onChunk) {
    return streamChatCompletion(systemPrompt, userPrompt, onChunk, apiKey, undefined, { type: 'json_object' });
  }
  return chatCompletion(systemPrompt, userPrompt, apiKey);
}
